...
- `tokensFile` 数据源语义与此相同,仅一处差异:本地文件没有「失联」概念——读取失败时保留最后一份快照继续放行,`snapshotMaxAge` 对它不适用(文件长期不可读也**不会** fail-close 新登录;快照数据源失联超时才会)
...
**凭证模型(2026-08-19 码直登;2026-08-27 mtunnel 平台 v4.3 起登录身份换为会话子密钥)**。`Login.user` 对 frps 始终是一个不透明身份串(子密钥 `mtk-` + base64url,≈47 字符;授权码同形但前缀为 `mt-`,不进登录链):frpc 配置 `user = <该串>`、`auth.token = <该串>`,**原文透传**——frps 不做任何翻译,验证方按该字段查表即可。mtunnel 平台下该串为控制面按 (授权码, 应用) 发放的会话子密钥(持授权码不即登录:终端持码向控制面解析应用时换取,仅驻内存);独立部署可自定为任意用户凭证。
